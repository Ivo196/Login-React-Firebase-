...
import { Alert } from './Alert';
  ...
  export function Login() {
    ...
    const {login, loginWithGoogle, resetPassword} = useAuth() //De ese objeto quiero obtener el signup
    ...
    const handleResetPassword = async() => {
      ...
      try {
        ...
        setError("We sent you an email with a link to reset your email")
      } catch (error) {
        setError(error.message)
      } 
    }
      ...
      return (
        ...
        {error && <Alert message={error}/>}
          ...
          <div className='mb-4'>
            <label htmlFor="email" className='block text-gray-700 text-sm front-fold mb-2'>Email</label>
        ...
        <button onClick={handleGoogleSingIn} className='bg-slate-50 hover:bg-slate-200 text-black shadow-md rounded border-2 border-gray-300 py-2 px-4 w-full'>Google Login</button>
        </div>
      )
    }
